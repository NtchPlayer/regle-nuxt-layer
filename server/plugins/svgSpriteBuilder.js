...
export default defineNitroPlugin((nitroApp) => {
  const svgDirectoryLayer = './public/svg/';
  ...
  const svgFileToSymbol = (svgFile) => {
    ...
    if (svgId.includes('svg')) {
      ...
      svgIdSplitBySlash = svgIdSplitBySlash.slice(
        svgIndex + 1,
        svgIdSplitBySlash.length
      );
      svgId = svgIdSplitBySlash.join('/');
    }
    return svg
      .replace('<svg', `<symbol id=${svgId}`)
      .replace('</svg>', '</symbol>');
  };

  const buildSvgSprite = () => {
    const svgFiles = getSvgFiles('./public/svg/');
